...
#geolocator = Nominatim("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36")
# db.add_product(pr_name= "Чизбургер", pr_quantity=10, pr_price=20000.0, pr_des="лучший", pr_photo="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTzolUu5EuGCYGg--0U4LV8vuQ0w1nHKxvMjiPgIqxCSA&s")
...
def settings(message):
    user_id = message.from_user.id
    bot.send_message(user_id, "Изменить номер телефона:", reply_markup=bt.settings_kb())
    bot.register_next_step_handler(message, request_new_phone_number)

def request_new_phone_number(message):
    user_id = message.from_user.id
    bot.send_message(user_id, "Введите ваш новый номер телефона:")
...
def update_phone_number(message):
    ...
    bot.send_message(user_id, "Номер телефона успешно обновлен!")


@bot.callback_query_handler(lambda call: call.data in ["back", "user_cart", "plus", "minus", "none", "to_cart", "clear_cart", "main_menu", "order"])
def for_call(call):
    user_id = call.message.chat.id
    if call.data == "back":
        bot.delete_message(user_id, call.message.message_id)
        bot.send_message(user_id, "Отправьте геолокацию или выберите адрес", reply_markup=bt.location_kb())
        ...
        bot.edit_message_reply_markup(chat_id=user_id, message_id=call.message.id, reply_markup=bt.exact_product(current_ammount, "plus"))
    elif call.data == "minus":
        current_ammount = users[user_id]["pr_count"]
        if current_ammount > 1:
            users[user_id]["pr_count"] -= 1
            bot.edit_message_reply_markup(chat_id=user_id, message_id=call.message.id, reply_markup=bt.exact_product(current_ammount, "minus"))
        else:
            pass
    elif call.data == "none":
        pass
        ...
        for i in user_cart:
            ...
            total_amount += i[2]
        full_text += f"\n\nИтоговая сумма {total_amount}"
        bot.send_message(-4106067936, full_text)
