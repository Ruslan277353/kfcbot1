...
def add_product(pr_name, pr_quantity, pr_price, pr_des, pr_photo):
    connection = sqlite3.connect("kfc.db")
    sql = connection.cursor()
    sql.execute("INSERT INTO products (pr_name, pr_price, pr_quantity, pr_des, pr_photo, reg_date) VALUES (?,?,?,?,?,?);",
...
def get_user_cart(user_id):
    connection = sqlite3.connect("kfc.db")
    sql = connection.cursor()
    user_cart = sql.execute("SELECT pr_name, pr_count, total_price FROM cart WHERE user_id=?;", (user_id,)).fetchall()
    return user_cart
...
def update_phone_number(user_id, new_phone_number):
    connection = sqlite3.connect("kfc.db")
    sql = connection.cursor()
    sql.execute("UPDATE users SET phone_number = ? WHERE user_id = ?", (new_phone_number, user_id))
    connection.commit()
